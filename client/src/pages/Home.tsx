...
export default function Home() {
  ...
  const storefrontInfo = React.useMemo(() => {
    try {
      return siteSettings.storefront ? JSON.parse(siteSettings.storefront) as {
  ...
  const isStoreOpen = React.useMemo(() => {
    if (!storefrontInfo?.hours) return null;
    const now = new Date();
    const dayNames = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
    const todayH = storefrontInfo.hours[dayNames[now.getDay()]];
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section
        ...
        style={{
          backgroundImage: storefrontInfo?.heroImageUrl ? `url(${storefrontInfo.heroImageUrl})` : undefined,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
          backgroundColor: storefrontInfo?.heroImageUrl ? undefined : '#0f172a',
        }}
      >
        <div className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/60 to-black/20" />
        <div className="relative z-10 container mx-auto px-6 py-16 md:py-24">
          <div className="max-w-xl">
            <h1 className="font-display text-5xl md:text-7xl font-bold text-white mb-3 tracking-tight">GridMart</h1>
            {storefrontInfo?.description && (
              <p className="text-white/80 text-lg mb-5">{storefrontInfo.description}</p>
            )}
            <div className="space-y-2 mb-5">
              {(storefrontInfo?.street || storefrontInfo?.city) && (
                <div className="flex items-center gap-2 text-white/90">
                  <MapPin className="w-4 h-4 text-primary shrink-0" />
                  <span className="text-sm">{[storefrontInfo.street, storefrontInfo.city, storefrontInfo.province, storefrontInfo.postalCode].filter(Boolean).join(', ')}</span>
                </div>
              )}
              {storefrontInfo?.phone && (
                <div className="flex items-center gap-2 text-white/90">
                  <Phone className="w-4 h-4 text-primary shrink-0" />
                  <span className="text-sm">{storefrontInfo.phone}</span>
                </div>
              )}
            </div>
            {isStoreOpen !== null && (
              <div className="flex items-center gap-3 mb-6">
                <Badge className={isStoreOpen ? 'bg-green-500 hover:bg-green-500 text-white border-0' : 'bg-red-500 hover:bg-red-500 text-white border-0'}>
                  {isStoreOpen ? 'Open Now' : 'Closed'}
                </Badge>
                {storefrontInfo?.hours && (() => {
                  const dayNames = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
                  const todayH = storefrontInfo.hours![dayNames[new Date().getDay()]];
                  if (!todayH || todayH.closed) return <span className="text-white/60 text-sm">Closed today</span>;
                  return <span className="text-white/60 text-sm">Today: {to12h(todayH.open)} – {to12h(todayH.close)}</span>;
                })()}
              </div>
            )}
            <div className="flex flex-wrap gap-3">
              <Button
                size="lg"
                className="bg-primary hover:bg-primary/90 text-white font-display"
                onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })}
              >
                Shop Now
              </Button>
              {(storefrontInfo?.street || storefrontInfo?.city) && (
                <Button size="lg" variant="outline" className="border-white/50 text-white hover:bg-white/10 font-display" asChild>
                  <a
                    href={`https://maps.google.com/?q=${encodeURIComponent([storefrontInfo.street, storefrontInfo.city, storefrontInfo.province, storefrontInfo.postalCode].filter(Boolean).join(', '))}`}
                    target="_blank"
      ...
      {/* Also available nearby — node pickup locations */}
      {activeNodes.length > 0 && (
        <section className="py-12 bg-muted/30" id="locations">
          <div className="container mx-auto px-4">
            <div className="mb-6">
              <h2 className="font-display text-2xl md:text-3xl font-bold mb-1">Also available nearby</h2>
              <p className="text-muted-foreground text-sm">Pick up orders from a neighbourhood node close to you.</p>
            </div>
            <div data-testid="homepage-pickup-map">
              <div className="relative group">
                ...
                <div
                  ...
                  className="overflow-hidden shadow-sm rounded-xl h-[280px] md:h-[380px] border border-primary"
                />
              ...
              {mapSelectedNode && (
                <div className="mt-4 p-4 bg-white/90 backdrop-blur-sm rounded-xl border shadow-sm max-w-2xl mx-auto">
                  ...
                  {mapSlotsLoading ? (
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">No pickup windows right now. Check back later!</p>
                  ) : (
