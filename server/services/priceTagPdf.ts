...
function renderTagCell(tag: PriceTagData, tmpl: PriceTagTemplate, autoFitText = false, autoFitPadding = 0): string {
  ...
  return `<div style="position:relative;width:${widthPx}px;height:${heightPx}px;background:white;overflow:hidden;outline:1px dashed rgba(0,0,0,0.25);outline-offset:-1px;">
    ${imgEl?.visible && tag.imageUrl ? `<div style="position:absolute;left:${imgEl.x}px;top:${imgEl.y}px;width:${imgEl.width}px;height:${imgEl.height}px;"><img src="${escapeHtml(tag.imageUrl)}" style="width:100%;height:100%;object-fit:contain;"></div>` : ''}
    ...
    ${name?.visible ? `<div ${autoFitText ? `data-autofit data-boxw="${name.width}" data-boxh="${name.height}" data-pad="${autoFitPadding}" data-bold="true" ` : ''}style="position:absolute;left:${name.x}px;top:${name.y}px;width:${name.width}px;height:${name.height}px;font-size:${tag.nameFontSize ?? name.fontSize}px;overflow:hidden;display:flex;align-items:flex-start;justify-content:${justify(name)};"><span style="font-weight:bold;font-family:Arial,sans-serif;line-height:1.2;word-break:break-word;width:100%;text-align:${name.textAlign||'left'};">${escapeHtml(tag.name)}</span></div>` : ''}
...
function generateTiledHTML(entries: { tag: PriceTagData; tmpl: PriceTagTemplate }[], autoFitText = false, autoFitPadding = 0): string {
  ...
  const sheetsHtml = sheetGroups.map((group, si) => {
    const cells = group
      .map(e => e ? renderTagCell(e.tag, e.tmpl, autoFitText, autoFitPadding) : `<div style="width:${widthPx}px;height:${heightPx}px;outline:1px dashed rgba(0,0,0,0.15);outline-offset:-1px;"></div>`)
      .join('');
  ...
  const fitScript = autoFitText
    ...
    : '';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;background:white;}@page{size:8.5in 11in;margin:0;}</style></head><body>${sheetsHtml}${fitScript}</body></html>`;
}
...
export async function generatePriceTagPdfs(
  ...
  const html = generateTiledHTML(entries, autoFitText, autoFitPadding);
